...
public:
    FunCallStatement(std::string name, std::vector<std::unique_ptr<Expression>> args);
...
public:
    SeqStatement(std::unique_ptr<Statement> first, std::unique_ptr<Statement> second);
...
class Definition {
public:
    Definition(std::string name, std::vector<std::string> args, std::unique_ptr<Statement> body);
...
class Program_Value {
public:
    explicit Program_Value(std::vector<llvm::Function*> definitions);
...
class Program {
public:
    Program(std::vector<std::unique_ptr<Definition>> definitions, std::unique_ptr<Statement> body);

    Program_Value* codegen();
