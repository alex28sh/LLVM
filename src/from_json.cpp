...
BinOp binop_from_json(json j) {
    static const std::unordered_map<std::string, BinOp> binOps = {
        {"Sub", BinOp::Sub}, {"Add", BinOp::Add}, {"Div", BinOp::Div}, {"Mul", BinOp::Mul}, {"Mod", BinOp::Mod},
        {"Eq", BinOp::Eq}, {"Neq", BinOp::Neq}, {"Gt", BinOp::Gt}, {"Ge", BinOp::Ge}, {"Lt", BinOp::Lt}, {"Le", BinOp::Le},
        {"And", BinOp::And}, {"Or", BinOp::Or}
    };
    auto it = binOps.find(j.get<std::string>());
    if (it != binOps.end()) {
        return it->second;
    } else {
...
std::unique_ptr<Expression> expr_from_json(json j) {
    ...
    if (type == "BinExpr") {
        ...
        return std::make_unique<BinExpr>(binop_from_json(contents[0]), l, r);
    }
    if (type == "UnExpr") {
        auto e = expr_from_json(contents[1]);
        return std::make_unique<UnExpr>(unop_from_json(contents[0]), e);
    }
    if (type == "FunCall") {
        ...
        std::transform(args_json.begin(), args_json.end(), args.begin(), expr_from_json);
...
std::unique_ptr<Statement> stmt_from_json(json j) {
    ...
    if (type == "If") {
        ...
        return std::make_unique<IfStatement>(std::move(e), std::move(l), std::move(r));
    }
    if (type == "VarDecl") {
        std::string name = contents.get<std::string>();
...
std::unique_ptr<Program> prg_from_json(json j) {
    ...
    std::transform(defs_json.begin(), defs_json.end(), defs.begin(), def_from_json);
